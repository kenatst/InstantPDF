...
final class ShareFlowTests: XCTestCase {
    ...
    func testTextFlowFromProviderToValidPDF() async throws {
        ...
        let summary = try XCTUnwrap(ShareFlowModel.ReadySummary.build(for: extracted))
        XCTAssertEqual(summary.title, String(localized: "Text", bundle: LanguageManager.bundle))
    ...
    func testMultiImageFlowCountOrderExistenceAndPageCount() async throws {
        ...
        let summary = try XCTUnwrap(ShareFlowModel.ReadySummary.build(for: extracted))
        XCTAssertEqual(summary.title, String(localized: "plural.images \(3)", bundle: LanguageManager.bundle))
        XCTAssertEqual(summary.subtitle, String(localized: "Order preserved", bundle: LanguageManager.bundle))
